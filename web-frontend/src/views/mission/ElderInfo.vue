    ...
    <div>
      <el-card class="user-table">
        ...
        <el-table
          ...
          <el-table-column prop="gender" label="性别" key="gender"
                           header-align="center"></el-table-column>
          <el-table-column prop="age" label="年龄" key="age"
                           header-align="center"></el-table-column>
          <el-table-column prop="status" label="身体状态" key="status"
                           header-align="center">
            <template slot-scope="scope">
              <el-tag size="middle" :type="tagColor(scope.row.status)">{{ scope.row.status }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="family" label="家庭情况" key="family"
                           header-align="center">
          </el-table-column>
          <el-table-column prop="pos" label="居住地点" key="pos"
                           header-align="center">
          </el-table-column>
...
<script>
import {addElderInfo, deleteElderInfo, getElderInfo, updateElderInfo} from "@/api";
...
export default {
  ...
  mounted() {
    this.refresh();
    this.getLocalStorageOfName();
  }
  ,
  methods: {
    //检查是否有name缓存
    getLocalStorageOfName() {
    ...
    clickSubmit() {
      if (this.opMode === 0) {
        //新增逻辑
        addElderInfo(this.dialogForm).then(res => {
          if (res.data === true) {
      ...
      if (this.opMode === 1) {
        //编辑逻辑
        updateElderInfo(this.dialogForm).then(res => {
          if (res.data === true) {
